...
## اگر exe پنجره را باز نمی‌کرد

سه علت اصلی بود و در کد برطرف شده است:

1. صفحه اصلی عکس `clinic.png` را به‌صورت اجباری از منابع داخلی می‌خواند؛ فایل وجود نداشت و برنامه قبل از باز شدن پنجره کرش می‌کرد.
2. مسیر عکس/PDF بعد از publish دنبال `DentalCenter.csproj` می‌گشت؛ روی exe نهایی این فایل نیست.
3. publish تک‌فایل بدون unpack کردن DLLهای native اسکیا (`libSkiaSharp`) پنجره را نشان نمی‌دهد.

الان عکس‌ها داخل اسمبلی embed شده‌اند، نبود فایل دیگر برنامه را نمی‌خواباند، و `build-exe.ps1` با
`IncludeNativeLibrariesForSelfExtract` خروجی می‌گیرد.

**خروجی درست:** `publish\DentalCenter.exe` بعد از `./build-exe.ps1`  
فقط فایل `bin\Debug\...\DentalCenter.exe` را تنها کپی نکنید.
