...
info/             مستند جامع ۱۲۰ صفحه‌ای نرم‌افزار (PDF)
```
...
### مستند جامع نرم‌افزار

فایل `info/DentalCenter-Documentation.pdf` یک مستند فارسی ۱۲۰ صفحه‌ای در پنج فصل است که
تمام برنامه و ویژگی‌های آن را شرح می‌دهد: چکیده، فصل اول (مقدمه و معرفی مفاهیم)،
فصل دوم (پیشینه و پژوهش)، فصل سوم (روش‌های پیشنهادی)، فصل چهارم (معرفی نرم‌افزار)،
فصل پنجم (نتیجه‌گیری و افق‌های پیش رو) و منابع و مآخذ.

متن فصل‌ها در `tools/infodoc/` نگهداری می‌شود و مستند با دستور زیر بازتولید می‌شود
(جدول‌ها، فهرست موضوع‌ها و اعداد محاسباتی مستقیماً از کد برنامه خوانده می‌شوند):

```bash
pip install fpdf2 uharfbuzz
python3 tools/generate_info_pdf.py    # خروجی: info/DentalCenter-Documentation.pdf
```
